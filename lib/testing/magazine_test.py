...
class TestMagazine:
    ...
    def test_name_len(self):
        ...
        # Test for name longer than 16 characters
        with pytest.raises(Exception):
            magazine_1.name = "New Yorker Plus X"

        # Test for name shorter than 2 characters
        with pytest.raises(Exception):
            magazine_2.name = "A"
    ...
    def test_contributing_authors(self):
        """Magazine returns authors who have written more than 2 articles"""
        author_1 = Author("Carry Bradshaw")
        author_2 = Author("Nathaniel Hawthorne")
        magazine_1 = Magazine("Vogue", "Fashion")
        Article(author_1, magazine_1, "How to wear a tutu with style")
        Article(author_1, magazine_1, "Dating life in NYC")
        Article(author_1, magazine_1, "Fashion trends in Paris")
        Article(author_2, magazine_1, "The Art of Writing")

        assert len(magazine_1.contributing_authors()) == 1
